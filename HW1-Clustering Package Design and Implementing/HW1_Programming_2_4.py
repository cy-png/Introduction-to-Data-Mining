...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 24 23:09:56 2020
...
from HW1_Programming_1 import *
import time
import datetime

# import data
def read_dataset():
    dataset = pd.read_csv('image_segmentation.csv', sep = ',')
...
centroid_list_list = []
for i in range(10):
    cluster,centroid_list = test_kmeans(7, 'random')
    centroid_list_list.append(centroid_list)
endtime = datetime.datetime.now()#Caculate time
execution_time = (endtime - starttime).seconds
print('execution time:',execution_time)#Caculate time



dataset = list(cluster.values())
labels = []
data = []


def get_key (dict, value):
    return [h for h, v in dict.items() if v == value]    

for i in dataset:
    for j in i:
        data.append(j)
        labels.append(np.array(get_key(cluster, i)))

import itertools
labels = list(itertools.chain.from_iterable(labels))

from pandas.core.frame import DataFrame
c={"cluster":labels, "data" :data}
dataset_new = DataFrame(c)
print(dataset_new)
