...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 20 19:49:41 2020
...
for i in test_labels:
    num = num + 1
    test_labels_sample.append(i)
    if num >= 300:
        break
    
train_images_sample_list = []
for i in train_images_sample:
    train_images_sample_row = []
    for j in i:
        train_images_sample_row.extend(j)
    train_images_sample_list.append(train_images_sample_row)

#        train_images_row.extend(j)
#    train_images_list.append(train_images_row)
    

train_images_sample_dataframe=DataFrame(train_images_sample_list)

starttime = datetime.datetime.now()#Caculate time
sample_model = KMeans(n_clusters=10).fit(train_images_sample_dataframe)#K-Means
...
objective_function_value=sample_model.inertia_ #152992612959.03403
print('objective function value', objective_function_value)

train_labels_int = []
for i in train_labels_sample:
...
cluster_center,cluster_assign=Kmeans(array(train_images_sample_dataframe),10)
...
starttime = datetime.datetime.now()#Caculate start time
sum_dis = 0
n = np.shape(train_images_sample_dataframe)[0]
for m in range(10):
    for i in range(n):
        if cluster_assign[i][0] == m:
            sum_dis += sum(power(array(train_images_sample_dataframe)[i]-cluster_center[m],2))
...
'''
starttime = datetime.datetime.now()#Caculate start time
count_num_cluster = {k:0 for k in range(10)}
for i in range(10):
    count_num_cluster[i] = list(cluster_assign[:,0]).count(i)
...
sum_k = 0
for k in range(10):
    sum_k += count_num_cluster[k]*sum(power(cluster_center[k],2))
    
my_own_objective_function_value_new = sum_i - sum_k
print(my_own_objective_function_value_new)
endtime = datetime.datetime.now()#Caculate end time
print((endtime - starttime).seconds)#Caculate time
#703887537.9874536
...
for m in range(10):
    for i in range(n):
        if sample_cluster[i] == m:
            sum_dis += sum(power(array(train_images_sample_dataframe)[i]-sample_center_ids[m],2))
print('objective function value',sum_dis)
#686247045.6767125
