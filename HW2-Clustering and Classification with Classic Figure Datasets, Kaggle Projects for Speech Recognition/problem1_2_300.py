...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 22 10:14:36 2020
...
for i in images_train:
    num = num + 1
    images_train_sample.append(i)
    if num >= 300:
        break
images_train_sample = np.array(images_train_sample)

labels_train_sample = []
num = 0
for i in labels_train:
    num = num + 1
    labels_train_sample.append(i)
    if num >= 300:
        break
labels_train_sample = np.array(labels_train_sample)
...
#accuracy
crosstable_data = {'label':labels_train_sample,
...
#objective function value
sum_dis = 0
n = np.shape(images_train_sample)[0]
for m in range(10):
    for i in range(n):
        if cluster_assign[i][0] == m:
            sum_dis += sum(power(images_train_sample[i]-cluster_center[m],2))
